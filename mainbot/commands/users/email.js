...
module.exports = {
            ...
            if (subcommand === "inbox") {
                ...
                if (
                    ...
                    return interaction.reply({
                        embeds: [{
                            title: `Error :x:`,
                            description: `This is meant for security email that for example end in: ${domains[0]}. You likely entered the primary email. Use the security email to receive codes/messages`,
                            color: 0xff0000
                        }],
                        ephemeral: true
                    });
    ...
    // secureconfig can be missing for some users; treat as optional instead of erroring
    let isValidDomain = false;
    const secureDomain = (secureconfig && secureconfig.length > 0) ? secureconfig[0].domain : null;

    if (domains.includes(domain) || domain === secureDomain) {
        isValidDomain = true;
    }
