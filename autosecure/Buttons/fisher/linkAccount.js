const { TextInputStyle, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder } = require("discord.js");
  ...
  callback: async (client, interaction) => {
    try {
      ...
      if (server.server_id !== interaction.guildId) {
        ...
        .setColor('#d22b2b')
        .setDescription(`Please only use your bot in one server, to keep using <#${interaction.channelId}>, set your server here using /set server!${config.botslotslink ? `### If you wish to gain access to another bot slot, you can purchase it from [here](${config.botslotslink}) and use \`/redeem\` to redeem it.` : ''}`);
    
      const components = [];
      
      if (config.botslotslink && config.botslotslink.startsWith('http')) {
