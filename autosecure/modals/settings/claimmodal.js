...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        try {
            ...
            if (hit.embeds) {
                await handleAutosecureHit(client, interaction, hit, guildId, channelId, storedName, isBotOwner, enteredUsername);
                ...
                if (isBotOwner) {
                    await handleBotOwnerClaim(client, interaction, acc, uid, guildId, channelId, mcname, enteredUsername);
                } else if (user.split > 1) {
                    await handleSplitClaim(client, interaction, acc, uid, guildId, channelId, mcname, user, enteredUsername);
                } else if (user?.claiming === 1) {
                    await handleFullAccount(client, interaction, acc, uid, guildId, channelId, mcname, user, false, enteredUsername);
                } else {
                    await handleSSIDOnly(client, interaction, acc, guildId, channelId, mcname, user, false, enteredUsername);
                }
