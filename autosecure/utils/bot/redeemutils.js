  ...
  const { notifierWebhook, roleid, guildid, domains } = require("../../../config.json");
  ...
  async function redeemLicense(client, interaction, license, licenseData) {
    ...
    if (existingAccess.length === 0) {
      await queryParams("INSERT INTO autosecure(user_id, domain) VALUES(?, ?)", [interaction.user.id, domains[0]]);
      await queryParams("INSERT INTO secureconfig(user_id, domain) VALUES(?, ?)", [interaction.user.id, domains[0]]);
    }
    ...
    try {
      ...
      if (member) {
        ...
        console.warn(`Could not fetch guild member ${interaction.user.id} to assign role`);
      }
    } catch (roleError) {
...
**Access license:** /license`)
          ...
          .setFooter({ text: `Powered by damnognick` })
          .setTimestamp(); 
      } else {
      ...
      await autosecurelogs(null, "redeem", interaction.user.id, null, null, null, message);
