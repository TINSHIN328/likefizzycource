...
module.exports = async (loginCookie) => {
    try {
        ...
        // Add initial delay to allow cookie propagation
        await sleep(500);
...
async function xbldata(loginCookie, retryCount) {
    ...
    const maxRetries = 4; // Increased from 1 to 4 for better reliability
    
    // Add exponential backoff delay between retries (not on first attempt)
    if (retryCount > 0) {
        const delayMs = Math.min(1000 * Math.pow(2, retryCount - 1), 5000); // 1s, 2s, 4s, 5s max
        console.log(`[XBL] Waiting ${delayMs}ms before retry ${retryCount}...`);
        await sleep(delayMs);
    }
