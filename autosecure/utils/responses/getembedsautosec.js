...
async function getembedautosec(client, embed, acc, interaction, claimUsername) {
    try {
       // console.log(acc);
        ...
        const replacePlaceholders = (text, replacements) => {
            ...
            const replacements = {
                "%USERNAME%": claimUsername || acc?.newName || acc?.oldName || "",
                "%UID%": acc?.uid || "",
            ...
            const replaceWithMap = (text) => replacePlaceholders(text, replacements);
            ...
            const processObject = (obj) => {
                ...
                for (const [k, v] of Object.entries(obj)) {
                    result[k] = typeof v === 'string' ? replaceWithMap(v)
