...
try {
    ...
    if (!button) {
        ...
        return interaction.reply({
            content: `Button not found or invalid action, please DM me ASAP.`,
            ephemeral: true
        });
    }
        if (!button.callback) {
...
for (const [buttonProp, { permission: dbPermission, description }] of Object.entries(userperms)) {
    if (button[buttonProp] && interaction.user.id !== client.username) {
        ...
        if (button.adminOnly) {
            // Only bot owner or verified admins can use this button
            if (interaction.user.id !== client.username) {
                let users = await client.queryParams(`SELECT * FROM users WHERE user_id=? AND child=? AND admin=1`, [client.username, interaction.user.id]);
                if (users.length === 0) {
                    return interaction.reply({ content: `Invalid permissions! You must be an admin to use this.`, ephemeral: true });
                }
            }
        }
