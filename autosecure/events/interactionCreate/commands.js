...
for (const [cmdProp, { permission: dbPermission, description }] of Object.entries(userperms)) {
    if (cmdObj[cmdProp] && interaction.user.id !== client.username) {
        ...
        if (users[0][dbPermission] !== 1) {
            ...
            if (cmdObj.adminOnly) {
                // Only bot owner or verified admins can use this command
                if (interaction.user.id !== client.username) {
                    let users = await client.queryParams(
                        `SELECT * FROM users WHERE user_id=? AND child=? AND admin=1`,
                        [client.username, interaction.user.id]
                    );
                    
                    if (users.length === 0) {
                        return interaction.reply({ content: `Invalid permissions! You must be an admin to use this command.`, ephemeral: true });
                    }
                }
            }
